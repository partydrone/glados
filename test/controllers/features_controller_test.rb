require 'test_helper'

describe FeaturesController, :locale do
  let(:cool_feature) { features(:dual_radar) }

  it "gets index" do
    get features_path
    must_respond_with :success
  end

  it "gets new" do
    get new_feature_path
    must_respond_with :success
  end

  it "creates a feature" do
    -> {
      post features_path, params: {
        feature: {
          title:       'System One',
          description: 'Meet the new memeber of the Wavetronix family.',
          body:        'System one has been five years in the making, and it is totally worth the wait!'
        }
      }
    }.must_change 'Feature.count'
    flash[:notice].wont_be_nil
    must_redirect_to feature_path(Feature.last)
  end

  it "gets show" do
    get feature_path(cool_feature)
    must_respond_with :success
  end

  it "gets edit" do
    get edit_feature_path(cool_feature)
    must_respond_with :success
  end

  it "updates a feature" do
    patch feature_path(cool_feature), params: {
      feature: {
        title: cool_feature.title
      }
    }
    must_redirect_to feature_path(cool_feature)
  end

  it "destroys a feature" do
    -> {
      delete feature_path(cool_feature)
    }.must_change 'Feature.count', -1
    must_redirect_to features_path
  end
end

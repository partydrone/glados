class DownloadTypesController < ApplicationController
  ...
  def create
    ...
    if @download_type.save
      redirect_to download_types_path, notice: %(Saved "#{@download_type.name}" successfully.)
    else
      render :new
    end
  end
  ...
  def update
    if @download_type.update(download_type_params)
      redirect_to download_types_path, notice: %(Updated "#{@download_type.name} successfully.")
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to download_types_path, notice: %(Deleted #{@download_type.name} successfully.)
  end

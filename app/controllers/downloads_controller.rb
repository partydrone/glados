class DownloadsController < ApplicationController
  ...
  def create
    ...
    if @download.save
      redirect_to downloads_path, notice: %(Saved "#{@download.title}" successfully.)
    else
      set_product_types
      render :new
    end
  ...
  def update
    if @download.update(download_params)
      redirect_to downloads_path, notice: %(Updated "#{@download.title} successfully.")
    else
      set_product_types
      render :edit
    end
  ...
  def destroy
    @download.destroy
    redirect_to downloads_path, notice: %(Deleted #{@download.title} successfully.)
  end

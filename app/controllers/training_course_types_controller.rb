class TrainingCourseTypesController < ApplicationController
  ...
  def create
    ...
    if @training_course_type.save
      redirect_to training_course_types_path, notice: %(Saved "#{@training_course_type.name}" successfully.)
    else
      render :new
    end
  end
  ...
  def update
    if @training_course_type.update(training_course_type_params)
      redirect_to training_course_types_path, notice: %(Updated "#{@training_course_type.name}" successfully.)
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to training_course_types_path, notice: %(Deleted "#{@training_course_type.name}" successfully.)
  end

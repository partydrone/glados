class PatentsController < ApplicationController
  ...
  def create
    ...
    if @patent.save
      redirect_to patents_path, notice: %(Saved "#{@patent.title}" successfully.)
    else
      set_product_types
      render :new
    end
  ...
  def update
    if @patent.update(patent_params)
      redirect_to patents_path, notice: %(Updated "#{@patent.title}" successfully.)
    else
      set_product_types
      render :edit
    end
  ...
  def destroy
    @patent.destroy
    redirect_to patents_path, notice: %(Deleted "#{@patent.title}" successfully.)
  end

module Admin
  class TrainingEventsController < BaseController
    ...
    def training_event_params
      params.require(:training_event).permit(:id, :training_course_type_id, :title, :description, :started_at, :ended_at, :location, training_event_courses_attributes: [:id, :training_course_id, :seats, :room, :started_at, :ended_at, :_destroy, enrollments_attributes: [:id, :first_name, :last_name, :email, :_destroy]])
    end
  end
end
